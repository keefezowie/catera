...
declare p v1.payouts;cid uuid;key text;finger text;status text;old v1.payout_events;result jsonb:='[]';rowdata record;begin
 ...
 if action='settlement.run' then
  ...
  with candidates as (select id from v1.payouts where settlement_run_id is not null and provider_id is not null and status in('pending','pending_compliance','submitting') and (settlement_polled_at is null or settlement_polled_at<clock_timestamp()-interval '5 minutes') order by settlement_polled_at nulls first,created_at limit 20 for update skip locked), claimed as (update v1.payouts p set settlement_polled_at=clock_timestamp() from candidates c where p.id=c.id returning p.id,p.provider_id,p.recipient_request) select coalesce(jsonb_agg(to_jsonb(claimed)),'[]') into result from claimed;return result;
