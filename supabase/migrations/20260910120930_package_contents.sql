...
create or replace function v1.delivery(d v1.delivery_days) returns jsonb language sql stable set search_path='' as $$ select to_jsonb(d)||jsonb_build_object('portions',s.portions,'trial',s.snapshot->'trial','offer',jsonb_set(s.snapshot->'offer','{menus}',v1.resolve_contents(s.package_id,d.service_date,s.snapshot->'offer'->'menus',coalesce((s.snapshot->'offer'->>'contentRevision')::int,0))),'cutoff_at',v1.cutoff(s.snapshot->'offer',d.service_date),'canChange',(s.snapshot->'offer'->>'flexible')::boolean and d.status='scheduled' and v1.cutoff(s.snapshot->'offer',d.service_date)>clock_timestamp(),'meals',(select jsonb_agg(jsonb_build_object('meal',f.meal,'status',f.status) order by f.meal desc) from v1.fulfillments f where f.day_id=d.id)) from v1.subscriptions s where s.id=d.subscription_id $$;
