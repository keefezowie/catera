...
- Each enabled lunch/dinner menu has its own contents and optional calories (kcal), protein, carbohydrates and fat (g). Partial estimates are supported; blank is unavailable and zero is retained. No nutrition calculation or automatic dietary claims are introduced.
- The contents editor follows package basics, before price/schedule/flexibility and review. Its preview uses the same rendering as customer details. Reducing and restoring a component count within the editor restores the removed dish text. Changing dishes clears entered nutrition so the caterer can enter fresh estimates.
- Dated menus are selected by package, content revision, date and meal. They retain the purchased box composition or à la carte dish count. Initial nutrition is not copied into a dated menu. Missing dated nutrition displays as unavailable.
...
- Shared domain/database suite covers structured validation, direct RPC rejection, combined meals, purchased revisions, immutable snapshots, dated nutrition, conflicts, tenant authorization and legacy upgrade/payment activation.
...
- iOS and Android component suites cover complete contents, serving sizes, zero macros, unavailable dated nutrition and English copy.
