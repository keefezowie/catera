...
- The contents editor follows package basics, before duration/pricing, schedule/flexibility and review. Caterers enter one daily per-portion price and see the one-portion package total calculated from the selected duration; the derived total is not stored separately. Its preview uses the same rendering as customer details. Reducing and restoring a component count within the editor restores the removed dish text.
