...
Web and Expo discovery now separate a concise meal preview from the full contents renderer. Web cards emphasize caterer identity and duration, show composition counts (or structured à la carte dish counts), and lead with the base package total for one portion across the full duration. A per-meal comparison rate sits directly beneath it: single-meal packages use the daily price, while combined lunch/dinner packages divide that daily price across two meals. Quantity discounts, trials, promotions and service fees remain part of the later quantity-aware purchase flow. Cards continue to group delivery status, comparison and package navigation. Dish-name search still indexes full menu contents.
...
- The contents editor follows package basics, before duration/pricing, schedule/flexibility and review. Caterers enter one daily per-portion price and see both the one-portion package total and its per-meal equivalent; combined lunch/dinner packages divide the daily price by two. Neither derived value is stored separately. Its preview uses the same rendering as customer details. Reducing and restoring a component count within the editor restores the removed dish text.
